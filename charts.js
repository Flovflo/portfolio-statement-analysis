(function chartKit(global) {
  ...
  const COLORS = ['#19d0e8', '#44ccff', '#ffffff', '#7f7f7f', '#062f34', '#3f3f3f'];
  ...
  function horizontalBars(items, options = {}) {
    ...
    items.forEach((item, index) => {
      ...
      chart.append(el('rect', { x: 250, y, width: 430, height: 12, rx: 2, fill: '#282828' }));
  ...
  function addGrid(chart, margin, plot, max, min = 0) {
    for (let step = 0; step <= 4; step += 1) {
      ...
      chart.append(el('line', { x1: margin.left, x2: margin.left + plot.width, y1: y, y2: y, stroke: '#282828', 'stroke-width': 1 }));
  ...
  function heatColor(value) {
    if (value > 0.72) return '#19d0e8';
    if (value > 0.45) return '#44ccff';
    if (value > 0.22) return '#062f34';
    if (value > 0.03) return '#3f3f3f';
    return '#191919';
  }
